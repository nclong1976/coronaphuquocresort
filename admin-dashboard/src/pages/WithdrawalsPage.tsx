  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        {withdrawals.map((w) => (
          <div key={w.id} className="bg-slate-900 rounded-xl border border-slate-800 p-6 flex justify-between items-center">
            <div>
            ...
            {w.status === 'pending' && (
              <div className="flex gap-2">
                ...
                />
                <button onClick={() => approveMutation.mutate(w.id)} className="p-2 bg-green-600 rounded-lg">
                  <Check size={20} />
                </button>
                <button onClick={() => rejectMutation.mutate(w.id)} className="p-2 bg-red-600 rounded-lg">
                  <X size={20} />
                </button>
              </div>
            )}
